...
export function BackgroundLayer() {
  ...
  useEffect(() => {
    ...
    // Parallax carries no meaning under reduced-motion preference; skip entirely
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return

    const onMove = (e: MouseEvent) => {
      targetX = (e.clientX / window.innerWidth  - 0.5) * 28
      targetY = (e.clientY / window.innerHeight - 0.5) * 28
      // Only wake the RAF loop when there is new movement to animate
    ...
    const tick = () => {
      ...
      currentX += dx * 0.038   // slower lerp = heavier, more cinematic
      currentY += dy * 0.038
      if (parallaxRef.current) {
        parallaxRef.current.style.transform =
          `translate(${currentX.toFixed(2)}px, ${currentY.toFixed(2)}px)`
      }
      // Once movement is imperceptible, stop looping — mousemove will restart it
      if (Math.abs(dx) < 0.06 && Math.abs(dy) < 0.06) { settled = true; return }
    ...
    <>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .bg-blob-1 { filter: blur(38px) !important; }
          .bg-blob-2 { filter: blur(44px) !important; }
          .bg-blob-3, .bg-blob-4 { display: none !important; }
        }
      `}</style>
